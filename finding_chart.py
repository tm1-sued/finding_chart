...
def main():
    ...
    if len(arguments)>0:
        if len(arguments)==1:
            if arguments[0] in ['-h', '-help', '--h', '--help']:
                print("usage for using standart initialisation file <./init.yml> : "+program_name )
                print("usage for using individual initialisation file            : "+program_name + 'init_file.txt' )
                sys.exit()
            else:
                initdatafile = arguments[0]
        elif len(arguments)>1:
            print("... don't  understand input, to much arguments")
            print("... arguments entered: <%s>"%(arguments) )
            print("usage for using standart initialisation file <./init.yml>  : "+program_name )
            print("usage for using individual (YAML-type) initialisation file : "+program_name + ' init_file.yml' )
            sys.exit()
    else:
    	initdatafile = "./init.yml"
    try:
        with open(initdatafile, 'r') as ymlfile: 
            ip = yaml.load(ymlfile, Loader=yaml.BaseLoader)
    except FileNotFoundError:
        print('...file <%s> not found, aborting...'% initdatafile)
        sys.exit()
